...
class FacebookMMSTTSHandler(ModelHandler):
    ...
    async def generate(self, pipeline: Any, prompt: str, **kwargs) -> Any:
        ...
        def _generate():
            ...
            # Handle parameter name mapping for transformers compatibility
            if 'sample_rate' in kwargs:
                # Some models expect 'sampling_rate' instead of 'sample_rate'
                basic_params['sampling_rate'] = kwargs['sample_rate']
            
            # Only include other parameters that MMS-TTS actually supports
            supported_keys = ['max_new_tokens']
            ...
            try:
                ...
                if "unexpected keyword argument" in str(e) and "sample_rate" in str(e):
                    # Try without any parameters that might be causing issues
                    logger.warning(f"Parameter error with MMS-TTS, trying with minimal params: {str(e)}")
