    ...
    for (let paragraphIndex = 0; paragraphIndex < paragraphs.length; paragraphIndex++) {
        ...
        if (potentialChunk.length <= this.options.maxChunkLength) {
          ...
          if (currentChunk) {
            chunks.push({
              ...
              seed: baseSeed + Math.floor(Math.random() * 10000) // Randomize seed for each chunk
            });
          }
      ...
      if (currentChunk) {
        chunks.push({
          text: currentChunk,
          index: chunks.length,
          isParagraphBreak: paragraphIndex < paragraphs.length - 1, // Mark if this ends a paragraph
          seed: baseSeed + Math.floor(Math.random() * 10000) // Randomize seed for each chunk
        });
      }
    }
