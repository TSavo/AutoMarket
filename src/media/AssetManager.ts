...
/**
 * Enhanced Asset Manager with Provider Integration
 * Provides typesafe access to media assets and content association
 */
export class AssetManager {
   ...
   */
  constructor(databasePath?: string, providerRegistry?: ProviderRegistry) {
  ...
  /**
   * Associate an asset with content (replaces blog-specific linking)
   * @param assetId Asset ID to associate
   * @param contentId Content ID (text string, script, document, etc.)
   * @param contentType Type of content
   * @param contentTitle Optional title for the content
   */
  public async associateWithContent(
    assetId: string,
    contentId: string,
    contentType: 'text' | 'script' | 'document' | 'workflow',
    contentTitle?: string
  ): Promise<void> {
    this.checkInitialized();

    const asset = await this.store.getAssetById(assetId);
    if (!asset) {
    ...
    await this.store.updateAsset(updatedAsset);
    console.log(`Associated asset ${assetId} with ${contentType} content: ${contentId}`);
  }

  /**
   * Get assets associated with specific content
   * @param contentId Content ID to search for
   * @param contentType Optional content type filter
   */
  public getAssetsByContent(contentId: string, contentType?: 'text' | 'script' | 'document' | 'workflow'): BaseAsset[] {
    this.checkInitialized();

    const assets = this.store.getAllAssets();
    return assets.filter(asset => {
      const assoc = (asset as any).associatedContent;
      if (!assoc || assoc.id !== contentId) {
        return false;
      }
      return !contentType || assoc.type === contentType;
    });
  }
