...
export class MediaIngestService {
   ...
   * @param providerRegistry Optional provider registry for enhanced capabilities
   */
  constructor(assetManager: AssetManager, providerRegistry?: ProviderRegistry) {
  ...
  /**
   * Generate transcript for a video using provider system or fallback to legacy Whisper
   * @param filePath Path to the video file
  ...
  private async generateVideoTranscript(filePath: string, assetId: string): Promise<void> {
    try {
      ...
      // Try provider system first if available
      if (this.providerRegistry) {
        const transcriptResult = await this.generateTranscriptWithProvider(filePath, assetId);
        if (transcriptResult) {
          return; // Success with provider system
        }
      }

      // Fallback to legacy Whisper service
      console.log(`[STT] Falling back to legacy Whisper service`);
      ...
      // Generate transcript with legacy service
  ...
  /**
   * Generate transcript using the provider system
   * @param filePath Path to the video/audio file
   * @param assetId Asset ID to update with transcript
   * @returns Promise<boolean> - true if successful, false if failed
   */
  private async generateTranscriptWithProvider(filePath: string, assetId: string): Promise<boolean> {
    if (!this.providerRegistry) {
      return false;
    }

    try {
      // Find the best STT provider
      const sttProvider = await this.providerRegistry.findBestProvider(MediaCapability.SPEECH_TO_TEXT, {
        preferLocal: true // Prefer local providers for privacy
      });

      if (!sttProvider) {
        console.warn(`[STT] No speech-to-text providers available`);
        return false;
      }

      console.log(`[STT] Using provider: ${sttProvider.name} (${sttProvider.id})`);

      // Get the best model for this provider
      const models = sttProvider.getModelsForCapability(MediaCapability.SPEECH_TO_TEXT);
      if (models.length === 0) {
        console.warn(`[STT] No models available for ${sttProvider.name}`);
        return false;
      }

      const model = models[0]; // Use the first available model
      console.log(`[STT] Using model: ${model.name} (${model.id})`);

      // Generate transcript
      const result = await sttProvider.generate({
        capability: MediaCapability.SPEECH_TO_TEXT,
        modelId: model.id,
        parameters: {
          audio_file: filePath,
          language: 'auto',
      ...
      if (result.status === 'completed' && result.outputs && result.outputs.length > 0) {
        const transcriptOutput = result.outputs[0];
        const transcriptText = transcriptOutput.data?.toString('utf-8') || '';

        if (transcriptText.trim()) {
          // Update asset with transcript data
          const asset = await this.assetManager.getAssetById(assetId);
          if (asset && isVideoAsset(asset)) {
            // Add transcript to asset metadata
            const updatedAsset = {
              ...asset,
              ...
              transcriptMetadata: {
                ...
                modelId: model.id,
                confidence: transcriptOutput.metadata?.confidence || 0.9,
                language: transcriptOutput.metadata?.language || 'auto',
                wordTimestamps: transcriptOutput.metadata?.wordTimestamps || [],
                generatedAt: new Date().toISOString(),
                processingTime: transcriptOutput.metadata?.processingTime,
                cost: result.cost
              }
            };

            await this.assetManager.updateAsset(updatedAsset);
            console.log(`[STT] Successfully generated transcript for ${assetId} using ${sttProvider.name}`);
            return true;
          }
        }
      } else {
        console.warn(`[STT] Transcription failed:`, result.error);
        return false;
      }

    } catch (error) {
      console.error(`[STT] Error with provider-based transcription:`, error);
      return false;
    }
