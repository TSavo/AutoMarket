...
export class FalTextToVideoModel extends TextToVideoModel {
  ...
  private async createVideoFromUrl(url: string, metadata: any = {}): Promise<Video> {
    try {
      ...
      console.log(`[FalTextToVideo] Video saved to: ${localPath} (${(videoBuffer.length / 1024 / 1024).toFixed(2)} MB)`);
      
      // Use SmartAssetFactory to create Asset with automatic metadata extraction
      console.log(`[FalTextToVideo] Loading video asset with metadata extraction...`);
