...
export class FFMPEGCompositionBuilder {
  ...
  buildFilterComplex(): string {
    ...
    if (hasIntroOutro) {
      return this.buildConcatenationFilterComplex(indexMapping);
    } else {
      return this.buildPureOverlayFilterComplex(indexMapping);
    }
  }

  /**
   * Build filter complex for pure overlay composition (no concatenation)
   */
  private buildPureOverlayFilterComplex(indexMapping: any): string {
    const filterParts: string[] = [];
    
    // Format base videos
    for (let i = 0; i < this.state.videos.length; i++) {
      filterParts.push(`[${indexMapping.mainIndices[i]}:v]format=yuv420p[base${i}]`);
    }

    // Process overlays with sophisticated features
    ...
    // Handle audio mixing for multiple inputs
    if (this.state.overlays.length > 0) {
      ...
      const totalInputs = indexMapping.mainIndices.length + indexMapping.overlayIndices.length;
      filterParts.push(`${allAudioInputs}amix=inputs=${totalInputs}:duration=longest:normalize=0[${this.state.filterOptions.audioOutputLabel}]`);
    } else {
      // Single video, copy audio
   ...
   * Get all videos in the composition order for FFmpeg input
   * Order: prepend -> main -> append -> overlays
   */
  getAllVideos(): Video[] {
    return [
  ...
  /**
   * Get videos for concatenation (excluding overlays which come after)
   */
   ...
   * Get overlay videos
   */
  getOverlayVideos(): Video[] {
    return this.state.overlays.map(o => o.video);
  }

  /**
   * Get input index mapping for filter complex generation
   */
  private getInputIndexMapping() {
    let inputIndex = 0;
    
    // Concatenation videos get consecutive indices starting from 0
    ...
    // Overlays come after all concatenation videos
   ...
   * Get video buffers for API calls - correct order for FFmpeg
   */
  ...
  private buildConcatenationFilterComplex(
    ...
    if (allVideoIndices.length > 1) {
      // Format all videos for concatenation (ONLY the main videos, NOT overlays)
      ...
      // Apply overlays to the concatenated video if any - USE SOPHISTICATED PROCESSING
      if (this.state.overlays.length > 0) {
        ...
        this.state.overlays.forEach((overlay, i) => {
          ...
          // Build sophisticated overlay processing - EXACT COPY FROM MAIN OVERLAY LOGIC
          let overlayFilter = `[${overlayIndex}:v]`;
